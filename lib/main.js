...
function iterator( a, b, options ) {
	...
	var i;
	if ( !isInteger( a ) ) {
		throw new TypeError( format( '0Pg7w', a ) );
	}
	if ( !isInteger( b ) ) {
		throw new TypeError( format( '0Pg7x', b ) );
	}
	if ( a > b ) {
	...
	if ( arguments.length > 2 ) {
		if ( !isObject( options ) ) {
			throw new TypeError( format( '0Pg2h', options ) );
		}
		opts = copy( options, 1 );
		if ( hasOwnProp( opts, 'iter' ) ) {
			if ( !isNonNegativeInteger( opts.iter ) ) {
				throw new TypeError( format( '0Pg35', 'iter', opts.iter ) );
			}
		} else {
			opts.iter = MAX_VALUE;
		}
